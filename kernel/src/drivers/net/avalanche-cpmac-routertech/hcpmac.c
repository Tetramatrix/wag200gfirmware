...
static int cpswHalIsAddrType2(HAL_DEVICE *HalDev);
int cpswHalCpgmacAddressAddType2(HAL_DEVICE *HalDev, int Channel,int Index, int Valid, int Match);
...
static int MacAddressSet(HAL_DEVICE *HalDev)
  ...
  if(cpswHalIsAddrType2(HalDev))
     return cpswHalCpgmacAddressAddType2(HalDev,0,0,1,1);
...
static void ConfigApply(HAL_DEVICE *HalDev)
  ...
  if(cpswHalIsAddrType2(HalDev))
  {
	  cpswHalCpgmacAddressAddType2(HalDev,0,0,1,1);
  }
  else
  {
...
#define  CPGMAC_MacAddr_Lo(base)            (*(volatile bit32u *)(base+0x500))
#define  CPGMAC_MacAddr_Hi(base)            (*(volatile bit32u *)(base+0x504))
#define  CPGMAC_MacIndex(base)              (*(volatile bit32u *)(base+0x508))

#define  CPGMAC_MacAddrType(base)           (*(volatile bit32u *)(base+0x188))
#define CPGMAC_ADDR_TYPE_2                  0x00000200

#define CPGMAC_valid         20
#define CPGMAC_match_filt    19
#define CPGMAC_channel       16
#define CPGMAC_macsrcaddr0    8
#define CPGMAC_macsrcaddr1    0
...
static int cpswHalIsAddrType2(HAL_DEVICE *HalDev)
{
	...
	regType = CPGMAC_MacAddrType(base);
    return (regType & CPGMAC_ADDR_TYPE_2);
	
	
}
      
/**
 *  @ingroup CPMAC_Ext_Functions
 *  This function is used to add an Ethernet address for a Specific Channel.
 *
 *  Note 1:  Should be called after Open() for a Type-II address implementation.
 *
 *  @param HalDev      CPHAL module instance. (set by cpswHal<module>InitModule())
 *  @param Channel     Specified Channel.
 *  @param MacAddress  The supplied Mac Address. For a Mac Address of "00.B0.D0.10.80.C1", MacAddress will be
 *                     [0]:0x00, [1]:0xB0, [2]:0xD0, [3]:0x10, [4]:0x80, [5]:0xC1.
 *  @param Index       Used with Type-II Addressing. Specifies index into Address list. Valid values are 0-31.
 *  @param Valid       Used with Type-II Addressing. Specifies whether supplied address will be used for match/filtering decision.
 *  @param Match       Used with Type-II Addressing. If set address will be Matched, otherwise it will be filtered.
 *  @return  EC_NO_ERRORS (ok). <BR>
 *           Possible Error Codes: <BR>
 *           @ref Error_Codes "EC_VAL_INVALID_STATE"<BR>
 *           @ref Error_Codes "EC_VAL_INVALID_CH"<BR>
 */
 
int cpswHalCpgmacAddressAddType2(HAL_DEVICE *HalDev, int Channel,int Index, int Valid, int Match)
{
    bit32u base;
	unsigned char *MacAddress = &HalDev->MacAddr[0];

    if((Index < 0) || (Index > 31))
       return(EC_CPMAC|EC_VAL_INVALID_STATE);

    base = HalDev->dev_base;

    CPGMAC_MacIndex(base) = Index;

    CPGMAC_MacAddr_Hi(base) = ((MacAddress[3] << 24)  |
                               (MacAddress[2] << 16)  |
                               (MacAddress[1] <<  8)  |
                               (MacAddress[0] <<  0));


    CPGMAC_MacAddr_Lo(base) = (((Valid   & 0x1) << CPGMAC_valid)         |
                               ((Match   & 0x1) << CPGMAC_match_filt)    |
                               ((Channel & 0x7) << CPGMAC_channel)       |
                               (MacAddress[5]   << CPGMAC_macsrcaddr0)   |
                               (MacAddress[4]   << CPGMAC_macsrcaddr1));
