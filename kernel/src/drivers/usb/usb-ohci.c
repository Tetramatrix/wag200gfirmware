 ...
 */
/* Fixed Bug in argument being passed to hc_add_ohci
 * Both ohci_t **ohci and local ohci_t *ohci share the same name
 * Modified ohci_t **ohci to ohci_t **ohciptr to fix this.
 * Also,stored the allocated ohci structure in ohciptr*/
int __devinit
hc_add_ohci(struct pci_dev *dev, int irq, void *mem_base, unsigned long flags,
	    ohci_t **ohciptr, const char *name, const char *slot_name)
{
	char buf[8], *bufp = buf;
	ohci_t * ohci;
	int ret;
...
#if !defined(CONFIG_USB_OHCI_SA1111) && !defined(CONFIG_USB_NON_PCI_OHCI) && !defined(CONFIG_USB_PSP_HOST11)  && !defined(CONFIG_USB_PSP_HOST11_MODULE)
