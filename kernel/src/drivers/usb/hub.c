					...
					struct usb_port_status *portsts)
{
	...
	/* Adding Fix:make sure we are not violating the max.depth criteria */
	tier = 0 ;
	temp = hub;
	while( temp )
	{
	...
	printk("New Device detected on port %d ,Parent Hub tier:%d\n",port,tier);

	if(tier > 6 )
	{
	  printk("new Device attached to Hub at Tier>6,Ignoring it\n");
	   return;
	}

	/* Hub to which new device is attached is at Tier denoted by
	 * variable tier */
