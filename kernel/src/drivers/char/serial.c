...
#define SIO_REG_OFFSET 8   /* 32 bit aligned boundary (evm III)*/
#endif
/*
 *  Texas Instruments Avalanche ASIC
 *
 */
#if defined(CONFIG_AVALANCHE_SERIAL)
#define SIO_REG_OFFSET 4   /* 32 bit aligned boundary (evm III)*/
#endif

#if defined(CONFIG_AVALANCHE_SERIAL) || defined(CONFIG_MIPS_SEAD2)
static _INLINE_ unsigned int serial_in(struct async_struct *info, int offset)
{
  return (inb(info->port + (offset * SIO_REG_OFFSET)) & 0xff);  
}
...
static inline unsigned int serial_inp(struct async_struct *info, int offset)
{
        ...
        return (inb(info->port + (offset * SIO_REG_OFFSET)) & 0xff);
#else /* CONFIG_SERIAL_NOPAUSE_IO */
        return (inb_p(info->port + (offset * SIO_REG_OFFSET)) & 0xff);
#endif /* CONFIG_SERIAL_NOPAUSE_IO */
}

static inline void serial_out(struct async_struct *info, int offset, int value)
{
        outb(value, info->port + (offset * SIO_REG_OFFSET));      
}
			       ...
			       int value)
{
        ...
        outb(value, info->port + (offset * SIO_REG_OFFSET));
#else /* CONFIG_SERIAL_NOPAUSE_IO */
        outb_p(value, info->port + (offset * SIO_REG_OFFSET));
#endif  /* CONFIG_SERIAL_NOPAUSE_IO */
}

#else /* !CONFIG_AVALANCHE_SERIAL || CONFIG_MIPS_SEAD2 */
...
#endif
#endif /* CONFIG_AVALANCHE_SERIAL || CONFIG_MIPS_SEAD2 */
 ...
 * in....
 */
#if defined(CONFIG_MIPS_SEAD2) || defined(CONFIG_AVALANCHE_SERIAL)
        /* nothing */
#else
#define serial_inp(info, offset)		serial_in(info, offset)
#define serial_outp(info, offset, value)	serial_out(info, offset, value)
#endif /* CONFIG_MIPS_SEAD2 || CONFIG_AVALANCHE_SERIAL*/
...
static int startup(struct async_struct * info)
{
	...
	change_speed(info, 0);
        ...
        /* Avalanche D has no flow control on the serial port */

#if defined(CONFIG_MIPS_SEAD2) || defined(CONFIG_AVALANCHE_SERIAL)

#if defined(CONFIG_AVALANCHE_SERIAL_AUTOFLOW)
        serial_outp(info, UART_MCR, TI_AUTOFLOW_ENABLE);
#endif
#endif /* CONFIG_MIPS_AVALANCHE || CONFIG_MIPS_SEAD2*/
...
#if defined(CONFIG_MIPS_SEAD2) || defined(CONFIG_AVALANCHE_SERIAL)
        int avbaud;
#endif /* CONFIG_MIPS_AVALANCHE || CONFIG_MIPS_AVALANCHE_SERIAL */
	...
	if (baud == 38400 &&
		...
		if (baud == 134)
			...
			quot = (2*baud_base / 269);
                  ...
                  {
#if defined(CONFIG_AVALANCHE_SERIAL)
                    avbaud = (avalanche_get_vbus_freq()/baud);
#endif
#if defined(CONFIG_MIPS_SEAD2)
                    avbaud = ((3683400)/baud);
#endif
...
{
#if defined(CONFIG_AVALANCHE_SERIAL) || defined(CONFIG_MIPS_SEAD2)
	unsigned int status, tmout = 100000;
#else
