		...
		unsigned int index, unsigned int length, int datablock, unsigned int *next_index)
{
	...
	if(compressed) {
		...
		if(((zlib_err = zlib_inflateInit(&stream)) != Z_OK) ||
		 ...
		 lzma_err=decompress_lzma_7z(c_buffer, c_byte,buffer, &out_size,lzma_buffer);
		 if(lzma_err){
                        ERROR("lzma returned unexpected result 0x%x\n",lzma_err);
//	/*  add auto reboot */
//	(*(volatile unsigned int *)((KSEG1ADDR(0x08610900)) + 0x00000004)) &= ~(1<<10);
                 	bytes = 0;
                 } else
	                bytes = out_size;
#endif
...
#else
	if(!(lzma_buffer = (char *) vmalloc(16384))){
		ERROR("Failed to allocate lzma buffer\n");
		return -ENOMEM;
	}
#endif
