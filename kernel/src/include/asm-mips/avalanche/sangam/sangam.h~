 ...
 *--------------------------------------------------*/
#define AVALANCHE_ADSL_SUB_SYS_MEM_BASE       (KSEG1_ADDR(0x01000000)) /* AVALANCHE ADSL Mem Base */
#define AVALANCHE_BROADBAND_INTERFACE__BASE   (KSEG1_ADDR(0x02000000)) /* AVALANCHE BBIF */        
#define AVALANCHE_ATM_SAR_BASE                (KSEG1_ADDR(0x03000000)) /* AVALANCHE ATM SAR */
...
#define AVALANCHE_UART1_REGS_BASE             (KSEG1_ADDR(0x08610F00)) /* AVALANCHE UART 0 */
...
#define AVALANCHE_FSER_BASE                   (KSEG1_ADDR(0x08612000)) /* AVALANCHE FSER base */
#define AVALANCHE_INTC_BASE                   (KSEG1_ADDR(0x08612400)) /* AVALANCHE INTC  */
/* AVALANCHE CPMAC 1 - OHIO has only CPMAC 0*/
#define AVALANCHE_HIGH_CPMAC_BASE             (IS_OHIO_CHIP() ? AVALANCHE_LOW_CPMAC_BASE : (KSEG1_ADDR(0x08612800)) )
...
#define AVALANCHE_HIGH_CPMAC_INT                  (IS_OHIO_CHIP() ? AVALANCHE_LOW_CPMAC_INT : 33)
/* Use this value for the static sysIntTypePolarity */
#define AVALANCHE_HIGH_CPMAC_INT_CONST            33
...
/* OHIO has only CPMAC 0                          */
#define AVALANCHE_HIGH_CPMAC_RESET_BIT            (IS_OHIO_CHIP()? AVALANCHE_LOW_CPMAC_RESET_BIT : 21)
...
#define AVALANCHE_MII_SEL_REG         (KSEG1_ADDR(0x08611A08))
...
#define OHIO_MIPS_MAX_FREQ 211968000
#define OHIO_SYS_MAX_FREQ  125000000

#define REG32_BIT_MASK(hi, lo) (((~(~0 << (hi))) & (~0 << (lo))) | (1 << (hi)))
#define REG32_R(addr, hi, lo) \
        			 ((*(volatile unsigned long *)(addr) & REG32_BIT_MASK((hi), (lo))) >> (lo))

#define IS_OHIO_CHIP() (( (REG32_R( AVALANCHE_CVR,15,0 ) == 0x2b) || (REG32_R( AVALANCHE_CVR,15,0 ) == 0x18)) ? 1:0)

#define	AVALANCHE_OHIO_CLOCK_XX_PLLCSR   (AVALANCHE_CLOCK_CONTROL_BASE + 0x80)
#define	AVALANCHE_OHIO_CLOCK_XX_PLLM     (AVALANCHE_CLOCK_CONTROL_BASE + 0x90)
#define	AVALANCHE_OHIO_CLOCK_XX_PREDIV   (AVALANCHE_CLOCK_CONTROL_BASE + 0x94)
#define	AVALANCHE_OHIO_CLOCK_XX_POSTDIV  (AVALANCHE_CLOCK_CONTROL_BASE + 0x98)
#define	AVALANCHE_OHIO_CLOCK_XX_PLLCMD   (AVALANCHE_CLOCK_CONTROL_BASE + 0xb8)    
#define	AVALANCHE_OHIO_CLOCK_XX_PLLCMDEM (AVALANCHE_CLOCK_CONTROL_BASE + 0xc0)
#define	AVALANCHE_OHIO_CLOCK_XX_PLLSTAT  (AVALANCHE_CLOCK_CONTROL_BASE + 0xbc)
