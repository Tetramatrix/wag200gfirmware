...
#define ATM_KEEPALIVE  _IOW('a',ATMIOC_SPECIAL+6,int)
					/* Keep nas interfaces alive */
#define ATM_STOPTX      _IOW('a',ATMIOC_SPECIAL+4,struct atmif_sioc)
					/* Stop Tx on Sangam DSL */
#define ATM_FLUSHTX  _IOW('a',ATMIOC_SPECIAL+5,struct atmif_sioc)
					/* Flush Tx queues */
/*
...
struct atm_vcc {
    ...
    /* PANKAJ: Added this function for protocol inspection. */
    int (*protocol_inspector)(struct sk_buff *skb);
