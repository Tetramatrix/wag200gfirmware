...
#endif
#ifdef CONFIG_MIPS_AVALANCHE_STATIC_SKB
       __u32           ti_skb_id;
#endif
};
...
/**************************************************************************
 * FUNCTION NAME : ti_dev_alloc_skb
 **************************************************************************
 * DESCRIPTION   :
 *  Wrapper function which calls the allocation routine. 
 ***************************************************************************/
extern struct sk_buff *ti_alloc_skb(unsigned int size,int gfp_mask);
extern int num_of_buffers(char* buf);
static inline struct sk_buff *ti_dev_alloc_skb(unsigned int length)
{
	return ti_alloc_skb(length, GFP_ATOMIC);
}
