 ...
 *
 *	$Id: br_input.c,v 1.2 2006-03-14 08:28:26 jeff Exp $
 *
...
/* Joel */
extern int br_trigger_hook(struct sk_buff *skb);
...
void br_pass_frame_up(struct net_bridge *br, struct sk_buff *skb)
{
	...
	if((skb->mac.ethernet->h_proto== htons(ETH_P_IP)) &&  !(dest[0] & 1) && br_trigger_hook(skb)) {
		kfree_skb(skb);
		return;
	}
#endif
