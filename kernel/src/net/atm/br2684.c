...
// 3) Voice-Data coexistence - Protocol Inspection.
//
...
/* These define the protocol type that is running on the 2684 device. */
#define UNSPECIFIED_PROTOCOL                    0
#define PPPOE_PROTOCOL                          1
#define ROUTED_PROTOCOL_BRIDGED_ENCAPS          2
#define ROUTED_PROTOCOL_ROUTED_ENCAPS           3
...
/**************************************************************************
 * FUNCTION NAME : br2684_learn_protocol
 **************************************************************************
 * DESCRIPTION   :
 *  The function is used to learn the protocol being executed on top of the
 *  2684 device.
 ***************************************************************************/
static void br2684_learn_protocol (struct sk_buff *skb, struct br2684_vcc* brvcc)
{
    int                 plen = 0;
    ...
    /* Determine the type and length of the header as specified by the 2684
     * encapsulation. */
    switch (brvcc->encaps)
    {
        case BR2684_ENCAPS_LLC:
    ...
    ptr_data = skb->data + plen;

    /* Check if the packet is bridged/routed encapsulated. */
    if (bridged_encapsulation == 1)
    {
        /* Bridged Encapsulated. Skip the destination and source mac address
         * in the ethernet header and determine the protocol being executed. */
        ptr_data = ptr_data + 12;

        /* Determine the protocol type. */
        protocol = *((unsigned short *)ptr_data);

        printk ("DEBUG: Bridged Encapsulation and protocol type=0x%x.\n", protocol);

        /* Use the protocol in the ethernet header to determine the type of
         * protocol being executed. */
        if ((protocol == __constant_htons(ETH_P_PPP_DISC)) || (protocol == __constant_htons(ETH_P_PPP_SES)))
        {
            /* The protocol type for PPP packets will be either PPPoE Session or the PPPoE Discovery.
             * Initialize the protocol offset to skip the protocol length, Ethernet header and 6 bytes of 
             * the PPP header. */
            brvcc->protocol_type    = PPPOE_PROTOCOL;
            brvcc->protocol_offset  = plen + 14 + 6;
            brvcc->ip_protocol_id   = __constant_htons(0x0021);
            printk ("DEBUG: Device has been detected as a PPP Protocol 2684 device.\n");
        }
        else
        {
            /* For routed protocols the ethernet protocol type is either IP or ARP.*/
            if ((protocol == __constant_htons(ETH_P_IP)) || (protocol == __constant_htons(ETH_P_ARP)))
            {
                /* Initialize the protocol offset to skip the 12 bytes of Ethernet header. (Dst Src MAC) */
                brvcc->protocol_type    = ROUTED_PROTOCOL_BRIDGED_ENCAPS;                
                brvcc->protocol_offset  = plen + 12;
                brvcc->ip_protocol_id   = __constant_htons(ETH_P_IP);
                printk ("DEBUG: Device has been detected as a Routed Protocol 2684 device.\n");
            }
            else
            {
                printk ("ERROR: Unable to determine protocol type for bridged encapsulated: 0x%x.\n",protocol);
            }
        }
    }
    else
    {
        /* Routed Encapsulated. */
        printk ("Routed Encapsulation.\n");
        brvcc->protocol_type = ROUTED_PROTOCOL_ROUTED_ENCAPS;

        /* Initialize the protocol offset. There is NO Ethernet header, we start at the IP packet. */
        brvcc->protocol_offset  = plen;
...
/**************************************************************************
 * FUNCTION NAME : br2684_protocol_inspector
 **************************************************************************
 * DESCRIPTION   :
 ...
 *  PPPoA protocol inspection routine defined in the file pppoatm.c.
 ***************************************************************************/
static int br2684_protocol_inspector(struct sk_buff *skb)
{
    ...
    if (gIsVoiceEnabled == 1)
    {
        ...
        brvcc = BR2684_VCC(ATM_SKB(skb)->vcc);

        /* First Time: Have we already learnt what sort of protocol is being
         * run on the VCC. */
        if (brvcc->protocol_type == UNSPECIFIED_PROTOCOL)
        {
            /* No. Try and learn the protocol */
            br2684_learn_protocol (skb, brvcc);
        }

        /* Skip the protocol header. */
        ptr_data = skb->data + brvcc->protocol_offset;

        /* We now need to get the IP header. */
        if (brvcc->protocol_type == ROUTED_PROTOCOL_ROUTED_ENCAPS)
        {
            /* Routed encapsulation means all packets are IP packets. */
            ptr_ipheader = (struct iphdr *)ptr_data;
        }
        else
        {
            /* Control comes here for Routed(Bridged encapsulation) or PPP
             * packets. */
            protocol = *((unsigned short *)ptr_data);
            if (protocol == brvcc->ip_protocol_id)
            {
                /* IP Packet. */
...
static void br2684_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
{
	...
	if (skb != NULL) 
    {
         ...
         * encapsulations. Added check for brdev since there may be a race condition
           where the connection is closed and we receive packet here. MR10359*/
    	if (brdev && (skb_pull(skb, plen) != NULL) && (header_error == 0))
        {
...
*/
	...
	/* Check for Valid VPI and VCI */
    if(((atmvcc->vpi >255 || atmvcc->vpi <0)) ||
	   ((atmvcc->vci < 32) || (atmvcc->vci > 65536))) 
	{
        printk(KERN_ERR
		    "br2684: Invalid VPI/VCI\n");
		err = -ENXIO;
		goto error;
	}	 
	if (atmvcc->push == NULL) {
    ...
    /* PANKAJ: Added this function for voice-data coexistence */
	atmvcc->protocol_inspector = br2684_protocol_inspector;
    brvcc->protocol_type       = UNSPECIFIED_PROTOCOL; 
    brvcc->protocol_offset     = 0;
    brvcc->ip_protocol_id      = 0;
    printk ("DEBUG: Registering protocol inspector: 0x%p for VCC:0x%p\n", atmvcc->protocol_inspector, atmvcc);
