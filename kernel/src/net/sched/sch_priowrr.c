...
/* PANKAJ: This should NOT be global. */
#if 0
...
/* Default schedule pattern:- for medium priority queue and low priority queue.
   Indexed using medium priory queue weightmap.Each entry is used to set the
   schedule table. bit 9 corresponds to first entry in schedule table and we
   put a 2 if the bit is reset and 3 if it is set
   */
static const u16 sch_default_pattern[TCQ_MAX_ST+1] = { 0x3FF,0x3DF,0x37B,0x3F8,0x39C,0x155,0x63,0x7,0x84,0x20,0x0};
...
static __inline__ unsigned priowrr_classify(struct sk_buff *skb, struct Qdisc *sch)
{
    ...
    /* Jyothi Check for skb->priority set in wbridge for packets from wlan and classify the packet here */
	if (skb->priority == 0xFFFF)
		return PRIOWRR_EFQ1; /* EF1 */
    else if (skb->priority == 0xFAAD)
		return PRIOWRR_EFQ2; /* EF2 */
    else if (skb->priority == 0xFADD)
		return PRIOWRR_BEQ1; /* BE1 */
    else if (skb->priority == 0xFDDD)
		return PRIOWRR_BEQ2; /* BE2 */
...
static void priowrr_create_sch_table(struct priowrr_sched_data *q, int wt, int qid, int offset_flag)
{
	int i, j, offset;
	...
	for (i=0; (i<wt)&&(offset<TCQ_MAX_ST); i++)
	{
		if (offset_flag == 1)
		{
			for (j=offset; (q->sch_table[j]!=0)&&(j<TCQ_MAX_ST); j++);
			if (j < TCQ_MAX_ST)
			{
				offset = j;
				q->sch_table[offset] = qid;
			}
			continue;
		}		
		q->sch_table[offset++] = qid;
...
static int priowrr_tune(struct Qdisc *sch, struct rtattr *opt)
{
	...
	if ((wt_afq+wt_bfq) > TCQ_MAX_ST)
		return -EINVAL;
	memset(q->sch_table, 0, TCQ_MAX_ST);
	
/*
	if ((wt_afq-wt_bfq) > (TCQ_MAX_ST/2))
	{
		priowrr_create_sch_table(q, wt_bfq, PRIOWRR_BEQ2, 0);
		priowrr_create_sch_table(q, wt_afq, PRIOWRR_BEQ1, 1);
	}
	else
	{
		priowrr_create_sch_table(q, wt_afq, PRIOWRR_BEQ1, 0);
		priowrr_create_sch_table(q, wt_bfq, PRIOWRR_BEQ2, 1);
	}
*/
	
    for (i=0; i < TCQ_MAX_ST; i++)
    {
        temp = (((sch_default_pattern[wt_afq]) >> (TCQ_MAX_ST-i-1)) & 1);
        q->sch_table[i]= (temp==0) ? PRIOWRR_BEQ1:PRIOWRR_BEQ2;
    }

	printk("\n  sch_table:");
	for (i=0; i<(wt_afq+wt_bfq); i++)
		printk(" %d", q->sch_table[i]);
	printk("\n");
