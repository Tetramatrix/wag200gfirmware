...
{
    /* NSP Voice Data Coexistence: When a socket is being closed, check if it is a 
     * voice socket and if so de-register the voice port from the VOICE
     * management module. */ 
    ...
    if (sk != NULL)
    {
        if (sk->priority != 0)
        {
            extern int voice_deregister_protocol_port (unsigned int port);
            voice_deregister_protocol_port (sk->sport);
        }
    }
