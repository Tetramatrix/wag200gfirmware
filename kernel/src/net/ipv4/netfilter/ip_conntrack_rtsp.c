...
rtp_expect(struct ip_conntrack *ct)
{
    ...
    DEBUGP("====> RTP_EXPECT"); DUMP_TUPLE(&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
    DEBUGP("====> RTP_EXPECT"); DUMP_TUPLE(&ct->tuplehash[IP_CT_DIR_REPLY].tuple);
                     ...
                     struct ip_ct_rtsp_expect* prtspexp)
{
    ...
    while (off < tranlen)
    {
        ...
        //printk("rtsp_parse_transport: ptran %s\n", ptran+off);
                ...
                struct ip_conntrack* ct, enum ip_conntrack_info ctinfo)
{
    ...
    while (dataoff < datalen)
    {
        ...
        DEBUGP("udp transport found, ports=(%d,%hu,%hu)\n",
              ...
              ntohs(exp.help.exp_rtsp_info.loport),
              ntohs(exp.help.exp_rtsp_info.hiport));

        LOCK_BH(&ip_rtsp_lock);
        port = ntohs(exp.help.exp_rtsp_info.loport);
        hiport = port + 1;
        ...
        while (port <= exp.help.exp_rtsp_info.hiport) {
            ...
            {
                INFOP("ip_conntrack_expect_related loport failed (%d)\n", rc);
            }
            port++;
        }
        UNLOCK_BH(&ip_rtsp_lock);
