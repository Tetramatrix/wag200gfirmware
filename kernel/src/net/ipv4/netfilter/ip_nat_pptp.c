...
DEBUGP("<0> pptp_help.\n");
	...
	if (!((hooknum == NF_IP_POST_ROUTING && dir == IP_CT_DIR_ORIGINAL)
		...
		DEBUGP("<0> nat_pptp: Not touching dir %s at hook %s\n",
	...
	if(hooknum == NF_IP_POST_ROUTING && dir == IP_CT_DIR_ORIGINAL)
	{
		...
		DEBUGP("<0> pptp_nat_help: Masq   original CID=%d as MCID=%d\n", 
	...
	if(hooknum == NF_IP_PRE_ROUTING && dir == IP_CT_DIR_REPLY)
	{
		switch (msg = htons(ctlh->messageType)) {
			case PPTP_OUT_CALL_REPLY:
				...
				DEBUGP("<0> Changing incoming peer call ID from %d to %d\n", 
		...
		DEBUGP("<0> pptp_nat_help: Demasq original MPCID=%d as PCID=%d\n", 
			ntohs(pcid), ntohs(nat->call_id));
	}
...
DEBUGP("<0> gre_help.\n");
	...
	if(master == NULL || exp->help.exp_pptp_info.pptp_magic != PPTP_TCP_PORT) {
		DEBUGP("<0> gre_help: Not part of an established PPTP connection\n");
		UNLOCK_BH(&ip_pptp_lock);
		return NF_ACCEPT;
	} else {
	...
	if(!ct->nat.help.pptp_info.serv_to_client)
	{
		/* 
	 	 ...
	 	 */
		if(hooknum == NF_IP_POST_ROUTING && dir == IP_CT_DIR_ORIGINAL) {
			DEBUGP("<0> gre_help: outgoing CID=%d\n", ntohs(greh->call_id));
		}
		...
		/*
	 	 ...
	 	 */
			...
			if(greh->call_id != nat->mcall_id)
				DEBUGP("<0> Whoops!  Incoming call ID isn't what we expect "
					"(expected %d, recv %d)!\n", 
					ntohs(nat->mcall_id), ntohs(greh->call_id));
	
			DEBUGP("<0> gre_help: CT=%lx, master CT=%lx, MCID=%d, demasq CID=%d\n", 
	...
	else
	{
		...
		if(hooknum == NF_IP_PRE_ROUTING && dir == IP_CT_DIR_ORIGINAL) {
			DEBUGP("<0> Fixing up packets from server first!\n");
			...
			DEBUGP("<0> PPTP Mangling %p: DST to %u.%u.%u.%u\n",
				*pskb, NIPQUAD(newdst));
			...
			DEBUGP("<0> PPTP Mangling %p: SRC to %u.%u.%u.%u\n",
				*pskb, NIPQUAD(newsrc));
		}
	}
	UNLOCK_BH(&ip_pptp_lock);
...
static int __init init(void)
{
	...
	if (ret == 0) {
		...
		if (ret != 0)
			...
			DEBUGP("<0> PPTP netfilter NAT helper: registered\n");
	}
	return ret;
}
