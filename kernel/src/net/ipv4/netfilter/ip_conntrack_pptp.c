...
DEBUGP("<0> printk_pptp_hdr.\n");
	...
	printk("<0> %s", from_txt);
        printk("<0> %d.%d.%d.%d -> ", NIPQUAD(iph->saddr));
        printk("<0> %d.%d.%d.%d ", NIPQUAD(iph->daddr));
	printk("<0> LEN=%d TY=%d MC=%X", ntohs(pptph->packetLength),
	...
	if (ntohs(pptph->packetType) == PPTP_CONTROL_PACKET) {
		...
		switch(msg)
		{
			case PPTP_OUT_CALL_REQUEST:
				printk("<0> CID=%d", ntohs(pptpReq.ocreq->callID));
				break;
			case PPTP_IN_CALL_REQUEST:
				printk("<0> CID=%d", ntohs(pptpReq.icreq->callID));
				break;	
			case PPTP_OUT_CALL_REPLY:
				printk("<0> CID=%d PCID=%d", ntohs(pptpReq.ocack->callID),
					ntohs(pptpReq.ocack->peersCallID));
				break;
			case PPTP_WAN_ERROR_NOTIFY:
				printk("<0> PCID=%d", ntohs(pptpReq.wanerr->peersCallID));
				break;
	
			case PPTP_SET_LINK_INFO:
				printk("<0> PCID=%d", ntohs(pptpReq.setlink->peersCallID));
				break;
	
			case PPTP_CALL_DISCONNECT_NOTIFY:
				printk("<0> CID=%d", ntohs(pptpReq.disc->callID));
		}
		printk("<0> CTL=%s", (msg <= PPTP_MSG_MAX)? strMName[msg]:strMName[0]);
	}
...
{
DEBUGP("<0> printk_gre_hdr.\n");
	printk("<0> %s GRE: ", from_txt);
        printk("<0> %d.%d.%d.%d -> ", NIPQUAD(iph->saddr));
        printk("<0> %d.%d.%d.%d ", NIPQUAD(iph->daddr));
	printk("<0> PR=%X LEN=%d CID=%d", ntohs(greh->protocol),
...
DEBUGP("<0> put_gre_tuple.\n");
	if((tt = kmalloc(sizeof(struct tuple_table), GFP_ATOMIC)) == NULL)
	{
		DEBUGP("<0> put_gre_tuple: out of memory\n");
		return NULL;
	}
	...
	DEBUGP("<0> put_gre_tuple(): Master=%p ", master);
...
DEBUGP("<0> get_gre_tuple.\n");
	...
	DEBUGP("<0> get_gre_tuple(): FAILED to lookup tuple: ");
        DEBUGP("<0> %d.%d.%d.%d -> ", NIPQUAD(s_addr));
        DEBUGP("<0> %d.%d.%d.%d ", NIPQUAD(d_addr));
        DEBUGP("<0> CID=%d\n", call_id);
        return NULL;
}
...
DEBUGP("<0> clean_gre_tuple.\n");
	...
	for (e=l->next; e!=l; e=e->next) {
		...
		if(tt->master == master) {
			DEBUGP("<0> clear_gre_tuple(): master=%p ", master);
			DEBUG_DUMP_TUPLE(&(tt->tuple));
			list_del(e);
			kfree(tt);
			found = 1;
	...
	if(!found)
	{
		DEBUGP("<0> clear_gre_tuple(): FAILED to delete tuple: ");
		DEBUGP("<0> master = %p\n", master);
	}
}
...
DEBUGP("<0> gre_invert_tuple.\n");
	...
	DEBUGP("<0> Couldn't find reponse to ");
...
DEBUGP("<0> gre_packet.\n");
	...
	DEBUGP("<0> CT=%lx, Master=%lx, DIR=%s ", (unsigned long) conntrack, 
...
DEBUGP("<0> gre_new.\n");
	...
	{
		DEBUGP("<0> gre_new: Unexpected - don't have a tuple for this packet!\n");
		return 0;
	}
	...
	DEBUGP("<0> CT=%lx, Master=%lx, DIR=new ", (unsigned long) conntrack, (unsigned long) master);
	PRINTK_GRE_HDR("", iph, greh);
				...
				enum ip_conntrack_info ctinfo)
{
	...
	struct ip_conntrack_expect expect, *exp;
...
DEBUGP("<0> ip_inbound_pptp_tcp.\n");
	pptph = (struct pptp_pkt_hdr *) ((char *) iph + sizeof(struct iphdr) + sizeof(struct tcphdr));

	DEBUGP("<0> inbound_pptp_tcp(): CT=%lx, ", (unsigned long) ct);
	...
	switch (msg = htons(ctlh->messageType)) {
		...
		default:
			DEBUGP("<0> UNKNOWN inbound packet: ");
	...
	ct_pptp_info = &ct->help.ct_pptp_info;
	exp = &expect;
	exp_pptp_info = &exp->help.exp_pptp_info;
	...
	if(ip_conntrack_protocol_register(&ip_conntrack_protocol_gre) == 0)
		DEBUGP("<0> pptp: registered conntrack protocol GRE!\n");
	else
		DEBUGP("<0> pptp: failed to register conntrack protocol GRE!\n");
				...
				enum ip_conntrack_info ctinfo)
{
	...
	__u16				msg, *cid;
DEBUGP("<0> ip_outbound_pptp_tcp.\n");
	pptph = (struct pptp_pkt_hdr *) ((char *) iph + sizeof(struct iphdr) + sizeof(struct tcphdr));

	DEBUGP("<0> outbound_pptp_tcp(): CT=%lx, ", (unsigned long) ct);
	PRINTK_PPTP_HDR("", iph, pptph);
	...
	switch (msg = htons(ctlh->messageType)) {
		...
		default:
			DEBUGP("<0> UNKNOWN outbound packet: ");
			DEBUGP("<0> %s (TY=%d)\n", (msg <= PPTP_MSG_MAX)? strMName[msg]:strMName[0], msg);
			/* fall through */
	...
	/* Info for NAT */
	DEBUGP("<0> ip_outbound_pptp_tcp: original client call id: %d\n", *cid);
	ct->nat.help.pptp_info.call_id = *cid;

	DEBUGP("<0> ip_outbound_pptp_tcp(): ");
	DEBUGP("<0> %s, CT=%lx, CID=%d\n", strMName[msg], (unsigned long) ct,
		(cid ? ntohs(*cid) : 0));
...
DEBUGP("<0> help.\n");
	...
	if (tcph->rst || tcph->fin) {
		DEBUGP("<0> pptp: bringing down gre connection.\n");
		delete_connection(ct, ctinfo);
	}
	
	/* Not whole TCP header? */
	if (tcplen < sizeof(struct tcphdr) || tcplen < tcph->doff*4) {
		DEBUGP("<0> pptp: tcplen = %u\n", (unsigned)tcplen);
		return NF_ACCEPT;
	}
	...
	if (tcp_v4_check(tcph, tcplen, iph->saddr, iph->daddr,
		...
		DEBUGP("<0> pptp_help: bad csum: %p %u %u.%u.%u.%u %u.%u.%u.%u\n",
	...
	if ((void *) pptph >= (void *) ((iph) + len)) {
		DEBUGP("<0> pptp_help(): no TCP data in pkt\n");
		return NF_ACCEPT;
	}
	...
	/* if it's not a control message we can't do anything with it */
		...
		DEBUGP("<0> pptp_help(): not a control pkt\n");
		return NF_ACCEPT;
	}
...
{
	int err;
	DEBUGP("<0> PPTP netfilter connection tracking: ");
	if((err = ip_conntrack_helper_register(&pptp_out)))
		printk("<0> register failed!\n");
	else
		DEBUGP("<0> registered\n");
	pptp_proc_create();
	return err;
