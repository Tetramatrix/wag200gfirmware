                 ...
                 struct sk_buff** pskb, uint tranoff, uint tranlen)
{
    ...
    case pb_range:
        ...
        {
            rbuf1len = sprintf(rbuf1, "%hu", loport);
            rbufalen = sprintf(rbufa, "%hu-%hu", prtspexp->loport, prtspexp->loport+1);
...
expected(struct sk_buff **pskb, uint hooknum, struct ip_conntrack* ct, struct ip_nat_info* info)
{
    ...
    mr.rangesize = 1;
    /*
     * We want to manip the per-protocol, and the IPs.
     */
...
init(void)
{
    ...
    for (i = 0; (i < MAX_PORTS) && ports[i] != 0; i++)
    {
        ...
        //hlpr->name = tmpname;
        ip_nat_rtsp_helpers[i].name = tmpname;
